...
IF EXISTS(SELECT NAME FROM sys.objects WHERE name = 'SP_CREATE_MODEL_I' AND type='P')
BEGIN
	DROP PROCEDURE SP_CREATE_MODEL_I
END
GO

CREATE PROCEDURE SP_CREATE_MODEL_I
(	
	@TABLE VARCHAR(80)
)
AS
BEGIN
	...
	DECLARE @NAME VARCHAR(80) = 'SP' + SUBSTRING(@TABLE,3, LEN(@TABLE)) +'_I'
	...
	BEGIN
		...
		SELECT @ID = (SELECT MIN(ID) FROM #TB_TEMP WHERE FOR_PARAM=0)
		...
		IF EXISTS(SELECT ID FROM #TB_TEMP WHERE ID> @ID AND FOR_PARAM = 0)
		BEGIN
			SET @LAST_ROW=0
		END
		...
		IF(@COLUMN_NAME = @PK_NAME)
		BEGIN
			UPDATE #TB_TEMP SET FOR_PARAM=1 WHERE ID = @ID
		END
		ELSE
		BEGIN
			...
			IF(@LAST_ROW=0)
			BEGIN
				SET @SQL+= @TAB + @TAB+ '@P_' + @COLUMN_NAME + ',' + @BR 
			END
			ELSE
			BEGIN
				SET @SQL+= @TAB + @TAB + '@P_' + @COLUMN_NAME + @BR 
	...
	SET @SQL+= @TAB + 'SET @P_' + @PK_NAME + ' = @@IDENTITY' + @BR + @BR
...
GRANT EXECUTE ON SP_CREATE_MODEL_I TO PUBLIC
GO